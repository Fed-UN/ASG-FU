...
import torch
from flwr.common import Context, Scalar, NDArrays, parameters_to_ndarrays, ndarrays_to_parameters
...
def get_evaluate_fn(shared_state, early_stopping_rounds, min_delta, max_delta, group_id):
    ...
    def evaluate_fn(server_round: int, parameters: NDArrays, config: Dict[str, Scalar]):
        ...
        with open(os.path.join(log_dir, f"{group_id}_log.csv"), "a") as f:
            if os.stat(os.path.join(log_dir, f"{group_id}_log.csv")).st_size == 0:
