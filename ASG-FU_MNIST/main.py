...
def save_group_config(groups, output_dir, features, client_ids):
    ...
    for lbl, members in groups.items():
        ...
        # 按平均相似度排序
        sorted_scores = sorted(enumerate(avg_similarities), key=lambda x: x[1], reverse=True)

        # 选择聚合器（得分最高的客户端）
        top_score = sorted_scores[0][1]
        top_candidates = [members[i] for i, score in enumerate(avg_similarities) if score == top_score]
        aggregator = random.choice(top_candidates) if top_candidates else members[0]

        # 选择候选者（得分第二高的客户端）
        if len(sorted_scores) >= 2:
            second_score = sorted_scores[1][1]
            second_candidates = [members[i] for i, score in enumerate(avg_similarities) if score == second_score]
        else:
            second_candidates = []
